...
1. 在 DevEco Studio 中为本机创建或选择调试签名配置。
2. 复制 `signing.local.example.json` 为 `signing.local.json`。
3. 将 DevEco Studio 生成的签名对象填写到 `signing.local.json`。
4. 确认 `signing.local.json` 被 Git 忽略且未被跟踪。
5. 正常执行 `devecocli build`。

根目录 `hvigorfile.ts` 会在本地文件存在时注入签名配置。不要把签名配置写回受版本控制的 `build-profile.json5`。
